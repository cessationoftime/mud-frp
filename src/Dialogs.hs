...
type DialogResult = (WXCore.FileDialog (), Int)
...
eventDialogButtonClick :: Int ->  Event t DialogResult ->  Event t (WXCore.FileDialog ())
...
eventDialogOk :: Event t DialogResult ->  Event t (WXCore.FileDialog ())
eventDialogOk = eventDialogButtonClick WXCore.wxID_OK

-- filter the eventDialogResult into a cancel-button-only result
eventDialogCancel :: Event t DialogResult ->  Event t (WXCore.FileDialog ())
...
eventDialogOkFilePath eDialogFinish = do
    ...
    fromAddHandler (WXCore.fileDialogGetPath `mapIO` addGetDialogFile)
